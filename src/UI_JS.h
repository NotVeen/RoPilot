...
const translations = {
    en: {
        ...
        lbl_total_instances: "Active Instances",
        lbl_total_cpu: "Total CPU",
        ...
        lbl_notify_rejoin: "Auto-Rejoin Status",
    ...
    id: {
        ...
        lbl_total_instances: "Instance Aktif",
        lbl_total_cpu: "Total CPU",
        lbl_total_ram: "Total RAM",
        ...
        lbl_notify_rejoin: "Status Auto-Rejoin",
...
function updateOverallAnalytics(accounts) {
    try {
        let activeAccs = (accounts || []).filter(function(a) {
            return a && (a.Status === 1 || a.Status === 2 || a.Status === 3);
        });
        let count = activeAccs.length;
        let totalCpu = 0;
        let totalRam = 0;
        let maxUptime = 0;
        ...
        for (let i = 0; i < activeAccs.length; i++) {
            let a = activeAccs[i];
            totalCpu += (a.CpuUsage || 0);
            totalRam += (a.RamUsage || 0);
            if ((a.RuntimeSeconds || 0) > maxUptime) {
                maxUptime = a.RuntimeSeconds;
            }
        }

        let langInput = document.getElementById("setting-language");
        let lang = (langInput && langInput.value) ? langInput.value : "en";
        ...
        if (elRam) {
            if (totalRam >= 1024) {
                elRam.textContent = (totalRam / 1024).toFixed(2) + " GB";
            } else {
                elRam.textContent = Math.round(totalRam) + " MB";
            }
        }
...
window.renderAccounts = function (accounts) {
    try {
        ...
        if (canQuickUpdate) {
            ...
            updateOverallAnalytics(accounts);
            return;
        }
